...
console.log(instance.method2());
  ...
  const smallResult = await runBenchmark(
    "Small code highlight",
    async () => {
      await highlightCodeBlock(
        new TextEncoder().encode(codeSamples.small),
        "js",
      );
    },
    1000,
  ...
  const mediumResult = await runBenchmark(
    ...
    async () => {
      await highlightCodeBlock(
        new TextEncoder().encode(codeSamples.medium),
        "javascript",
      );
    },
    100,
  ...
  const largeResult = await runBenchmark(
    "Large code highlight",
    async () => {
      await highlightCodeBlock(
        new TextEncoder().encode(codeSamples.large),
        "js",
      );
    },
    10,
...
async function benchmarkBlockEvents() {
  const { blocks } = await import("../src/parser/block-parser.ts");
...
`.trim();
  ...
  const smallBytes = u8(smallDoc);
  const mediumBytes = u8(mediumDoc);
  const largeBytes = u8(largeDoc);

  const smallResult = await runBenchmark(
    "Small block events",
    () => {
      let count = 0;
      for (const ev of blocks(smallBytes)) {
        if (ev.type !== "paraLine") count++;
      }
      if (count === 0) throw new Error("no events parsed");
    },
    5000,
  );
  ...
  const mediumResult = await runBenchmark(
    ...
    () => {
      let count = 0;
      for (const ev of blocks(mediumBytes)) {
        if (ev.type !== "paraLine") count++;
      }
      if (count === 0) throw new Error("no events parsed");
    },
  ...
  const largeResult = await runBenchmark(
    ...
    () => {
      let count = 0;
      for (const ev of blocks(largeBytes)) {
        if (ev.type !== "paraLine") count++;
      }
      if (count === 0) throw new Error("no events parsed");
    },
...
async function benchmarkInlineParsing() {
  ...
  const inlineSamples = {
    ...
    complex:
      "Text with **bold**, *italic*, `code`, ~~strikethrough~~, [links](url), and ![images](src).",
    nested:
      "**Bold *italic **nested bold*** italic** and more ~~strikethrough **bold strike**~~",
    long:
      "Very long text ".repeat(100) +
      " with **bold** and *italic* formatting throughout.",
  };
  ...
  const simpleResult = await runBenchmark(
    ...
    () => {
      const tokens = Array.from(
        inlineTokens(u8(inlineSamples.simple), 0, inlineSamples.simple.length),
      );
    },
    10000,
  );
  ...
  const complexResult = await runBenchmark(
    "Complex inline parse",
    () => {
      const tokens = Array.from(
        inlineTokens(
          u8(inlineSamples.complex),
          0,
          inlineSamples.complex.length,
        ),
      );
    },
    1000,
  ...
  const nestedResult = await runBenchmark(
    ...
    () => {
      const tokens = Array.from(
        inlineTokens(u8(inlineSamples.nested), 0, inlineSamples.nested.length),
      );
    },
    1000,
  );
  ...
  const longResult = await runBenchmark(
    ...
    () => {
      const tokens = Array.from(
        inlineTokens(u8(inlineSamples.long), 0, inlineSamples.long.length),
      );
    },
    100,
  );
...
test("benchmark: parser performance", async () => {
  ...
  console.log(
    `Small document: ${results.smallResult.opsPerSec.toFixed(0)} ops/sec (${results.smallResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Medium document: ${results.mediumResult.opsPerSec.toFixed(0)} ops/sec (${results.mediumResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Large document: ${results.largeResult.opsPerSec.toFixed(0)} ops/sec (${results.largeResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
...
test("benchmark: highlighting performance", async () => {
  ...
  console.log(
    `Small code: ${results.smallResult.opsPerSec.toFixed(0)} ops/sec (${results.smallResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Medium code: ${results.mediumResult.opsPerSec.toFixed(0)} ops/sec (${results.mediumResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Large code: ${results.largeResult.opsPerSec.toFixed(0)} ops/sec (${results.largeResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
...
test("benchmark: block event generation", async () => {
  const results = await benchmarkBlockEvents();

  console.log("\n=== Block Event Benchmarks ===");
  console.log(
    `Small document events: ${results.smallResult.opsPerSec.toFixed(0)} ops/sec (${results.smallResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Medium document events: ${results.mediumResult.opsPerSec.toFixed(0)} ops/sec (${results.mediumResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Large document events: ${results.largeResult.opsPerSec.toFixed(0)} ops/sec (${results.largeResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
...
test("benchmark: inline parsing performance", async () => {
  ...
  console.log(
    `Simple inline: ${results.simpleResult.opsPerSec.toFixed(0)} ops/sec (${results.simpleResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Complex inline: ${results.complexResult.opsPerSec.toFixed(0)} ops/sec (${results.complexResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Nested inline: ${results.nestedResult.opsPerSec.toFixed(0)} ops/sec (${results.nestedResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
  console.log(
    `Long inline: ${results.longResult.opsPerSec.toFixed(0)} ops/sec (${results.longResult.avgTimeMs.toFixed(3)}ms avg)`,
  );
...
test("benchmark: memory usage", async () => {
  ...
  console.log(
    `Memory used for 100 large document parses: ${(memoryUsed / 1024 / 1024).toFixed(2)} MB`,
  );
