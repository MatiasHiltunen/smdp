...
import { BaseRenderEmitter, RenderBus } from "./render-emitter";
...
function renderInline(
  ...
  for (const tok of inlineTokens(u8, s, e)) {
    ...
    switch (tok.kind) {
      ...
      case "link": {
        const hrefText = new TextDecoder().decode(
          u8.subarray(tok.hrefS, tok.hrefE),
        );
        ...
        if (allowed) {
          ...
          renderInline(u8, tok.textS, tok.textE, out, options, bus);
      ...
      case "autolink": {
        ...
        const hrefText = tok.isWww
          ? "https://" + new TextDecoder().decode(u8.subarray(tok.s, tok.e))
          : new TextDecoder().decode(u8.subarray(hrefStart, hrefEnd));
        if (tok.isWww) {
 ...
 */
export async function renderHTMLFromBlocks(
  u8: Uint8Array,
  options: ParserOptions = {},
  emitter?: RenderEmitter,
): Promise<string> {
  const emitterInstance = emitter ?? new BaseRenderEmitter();
  const bus = new RenderBus(emitterInstance);
  const out = borrowHtmlArena();
  try {
    ...
    for (const ev of blocks(u8)) {
      ...
      switch (ev.type) {
        ...
        case "heading":
          ...
          renderInline(u8, ev.s, ev.e, out, options, bus);
        ...
        case "tableHeader":
          ...
          for (const cell of ev.cells) {
            ...
            renderInline(u8, cell.s, cell.e, out, options, bus);
    ...
    if (footnotes.length > 0) {
      ...
      for (const fn of footnotes) {
        ...
        out.writeAscii('">');
        renderInline(u8, fn.contentS, fn.contentE, out, options, bus);
