...
// Build once (lowercase ASCII) — no strings in hot path
const KW_BYTES = (() => {
  const list = [
    'break','case','catch','class','const','continue','debugger','default','delete','do','else','export','extends',
    'finally','for','function','if','import','in','instanceof','let','new','return','super','switch','this','throw',
    'try','typeof','var','void','while','with','yield','enum','await','implements','interface','package','private',
    'protected','public','static','as','from','of'
  ];
  return list.map(s => TE.encode(s)); // Uint8Array[]
})();

function isKWBytes(u8, s, e) {
  const len = e - s;
  outer: for (let i = 0; i < KW_BYTES.length; i++) {
    const w = KW_BYTES[i];
    if (w.length !== len) continue;
    for (let k = 0; k < len; k++) {
      // ASCII lowercasing without allocation
      const b = u8[s + k];
      const lc = (b >= 0x41 && b <= 0x5A) ? (b | 0x20) : b;
      if (lc !== w[k]) continue outer;
    }
    return [true, kwHotCodeBytes(u8, s, e)]; // same small code table
  }
  return [false, 0];
}

function kwHotCodeBytes(u8, s, e) {
  // Only branch on hot keywords; bytewise match to avoid decoding
  const eq = (str) => {
    if (e - s !== str.length) return false;
    for (let k = 0; k < str.length; k++) {
      let b = u8[s + k]; if (b >= 0x41 && b <= 0x5A) b |= 0x20;
      if (b !== str.charCodeAt(k)) return false;
    }
    return true;
  };
  if (eq('return')) return 1;
  if (eq('throw')) return 2;
  if (eq('case')) return 3;
  if (eq('typeof')) return 4;
  if (eq('void')) return 5;
  if (eq('delete')) return 6;
  if (eq('in')) return 7;
  if (eq('instanceof')) return 8;
  if (eq('new')) return 9;
  return 0;
}

// After which token types a '/' can start a regex literal
function canStartRegex(prevType, prevValCode, lastSig, lastTwo) {
  ...
  switch (prevType) {
    ...
    case TT.Punct:
    case TT.Operator:
      // Disallow after expression closers and postfix ops
...
export class JSTokenizer {
  ...
  tokenize(u8, emit){
    ...
    let prevType = null, prevKwCode = 0, lastSig = 0, lastTwo = 0;
    ...
    while (i < n){
      ...
      if (c === 0x23 /*#*/ && i+1<n && isIdentStart(u8[i+1])) {
        let s=i; i++; while (i<n && isIdentPart(u8[i])) i++;
        push(TT.Identifier, s, i); continue;
      }
      ...
      if (isIdentStart(c) || (c===0x5C && i+1<n && u8[i+1]===0x75)){
        ...
        for(;;){ if (i>=n) break; const d=u8[i]; if (isIdentPart(d)) { i++; continue; }
          ...
          break; }
        const [kw, code] = isKWBytes(u8,s,i);
      ...
      { let s=i; const ch=c; i++; const n1=(i<n)?u8[i]:0, n2=(i+1<n)?u8[i+1]:0;
        ...
        push(punct?TT.Punct:TT.Operator, s, i); 
        if (i === s + 1) {
          lastTwo = ((lastTwo & 0xFF) << 8) | ch;
          lastSig = ch;
        }
  ...
  // Fast-path multi-char ops (ASCII)
  function scanOperator(u8, i, n) {
    const a = u8[i], b = (i+1<n?u8[i+1]:0), c = (i+2<n?u8[i+2]:0);
    // 3-char first
    if (a===0x3E && b===0x3E && c===0x3E) return i+3;       // >>>
    if (a===0x3E && b===0x3E && c===0x3D) return i+3;       // >>=
    if (a===0x3C && b===0x3C && c===0x3D) return i+3;       // <<=
    if (a===0x2A && b===0x2A && c===0x3D) return i+3;       // **=
    if (a===0x3F && b===0x3F && c===0x3D) return i+3;       // ??=
    if (a===0x2E && b===0x2E && c===0x2E) return i+3;       // ...
    // 2-char
    if (a===0x2A && b===0x2A) return i+2;                   // **
    if (a===0x26 && b===0x26) return i+2;                   // &&
    if (a===0x7C && b===0x7C) return i+2;                   // ||
    if (a===0x3F && b===0x3F) return i+2;                   // ??
    if (a===0x3F && b===0x2E) return i+2;                   // ?.
    if (a===0x26 && b===0x3D) return i+2;                   // &=
    if (a===0x7C && b===0x3D) return i+2;                   // |=
    if (a===0x5E && b===0x3D) return i+2;                   // ^=
    if (a===0x2B && b===0x2B) return i+2;                   // ++
    if (a===0x2D && b===0x2D) return i+2;                   // --
    if (a===0x3D && b===0x3D) return i+2;                   // ==
    if (a===0x21 && b===0x3D) return i+2;                   // !=
    if (a===0x2A && b===0x3D) return i+2;                   // *=
    if (a===0x2F && b===0x3D) return i+2;                   // /=
    if (a===0x25 && b===0x3D) return i+2;                   // %=
    if (a===0x2B && b===0x3D) return i+2;                   // +=
    if (a===0x2D && b===0x3D) return i+2;                   // -=
    if (a===0x3D && b===0x3E) return i+2;                   // =>
    if (a===0x3C && b===0x3C) return i+2;                   // <<
    if (a===0x3E && b===0x3E) return i+2;                   // >>
    if (a===0x26 && b===0x3D) return i+2;                   // &&=
    if (a===0x7C && b===0x3D) return i+2;                   // ||=
    return i+1; // single-char op/punct handled by caller
  }

  *tokens(u8){
    const n = u8.length;
    let i = 0, prevType = null, prevKwCode = 0, lastSig = 0, lastTwo = 0;
    const push = (type, s, e, meta)=>{ prevKwCode = (type===TT.Keyword ? (meta|0) : 0); prevType = type; return [type,s,e,meta]; };

    if (i+1<n && u8[0]===0x23 && u8[1]===0x21){ let j=2; while(j<n && !isNL(u8[j])) j++; yield push(TT.Comment, 0, j); i=j; }

    while (i < n){
      const c = u8[i];
      if (isWS(c)) { let s=i; do{i++;}while(i<n && isWS(u8[i])); yield push(TT.Whitespace,s,i); continue; }
      if (isNL(c)) { let s=i; if (c===0x0D && i+1<n && u8[i+1]===0x0A) i+=2; else i++; yield push(TT.Newline, s, i); continue; }

      if (c === 0x23 /*#*/ && i+1<n && isIdentStart(u8[i+1])) {
        let s=i; i++; while (i<n && isIdentPart(u8[i])) i++;
        yield push(TT.Identifier, s, i); continue;
      }
      ...
      if (isIdentStart(c) || (c===0x5C && i+1<n && u8[i+1]===0x75)){
        ...
        for(;;){ if (i>=n) break; const d=u8[i]; if (isIdentPart(d)) { i++; continue; }
          ...
          break; }
        const [kw, code] = isKWBytes(u8,s,i);
        yield push(kw?TT.Keyword:TT.Identifier, s, i, code);
        continue;
      }
      ...
      if (isDigit(c) || (c===0x2E && i+1<n && isDigit(u8[i+1]))){
        ...
        if (i<n && u8[i]===0x6E) i++; // bigint n
        yield push(TT.LiteralNum, s, i); continue;
      }

      if (c===0x27 || c===0x22){ let s=i; const quote=c; i++; while(i<n){ const d=u8[i++]; if (d===quote) break; if (d===0x5C && i<n) i++; if (isNL(d)) break; } yield push(TT.LiteralStr, s, i); continue; }
      ...
      if (c===0x60){ let s=i; i++; while(i<n){ const d=u8[i++]; if (d===0x60) break; if (d===0x5C && i<n){ i++; continue; } if (d===0x24 && i<n && u8[i]===0x7B){
        ...
        yield push(TT.LiteralTpl, s, i); continue; }

      if (c===0x2F){ if (i+1<n){ const d=u8[i+1]; if (d===0x2F){ let s=i; i+=2; while(i<n && !isNL(u8[i])) i++; yield push(TT.Comment, s, i); continue; }
          if (d===0x2A){ let s=i; i+=2; while(i+1<n && !(u8[i]===0x2A && u8[i+1]===0x2F)) i++; i=Math.min(n,i+2); yield push(TT.Comment, s, i); continue; }
          if (canStartRegex(prevType, prevKwCode, lastSig, lastTwo)){ let s=i; i++; let inClass=false; while(i<n){ const x=u8[i++]; if (x===0x5C){ if(i<n) i++; continue; } if (x===0x5B){ inClass=true; continue; } if (x===0x5D){ inClass=false; continue; } if (x===0x2F && !inClass){ break; } if (isNL(x)) break; } while(i<n){ const f=u8[i]; if ((f>=0x61&&f<=0x7A)||(f>=0x41&&f<=0x5A)) i++; else break; } yield push(TT.Regex, s, i); continue; } }
        let s=i; i++; if (i<n && u8[i]===0x3D) i++; yield push(TT.Operator, s, i); continue; }

      { let s=i; const ch=c; i++; const n1=(i<n)?u8[i]:0, n2=(i+1<n)?u8[i+1]:0;
        let end = scanOperator(u8, s, n);
        if (end > s) {
          yield push(TT.Operator, s, end);
          if (end === s + 1) {
            lastTwo = ((lastTwo & 0xFF) << 8) | ch;
            lastSig = ch;
          }
        ...
        const punct = (ch===0x28||ch===0x29||ch===0x5B||ch===0x5D||ch===0x7B||ch===0x7D||ch===0x2C||ch===0x3B||ch===0x3A||ch===0x2E);
        yield push(punct?TT.Punct:TT.Operator, s, i); 
        if (i === s + 1) {
          lastTwo = ((lastTwo & 0xFF) << 8) | ch;
          lastSig = ch;
        }
...
export class JSTokenizerState {
  constructor(){
    ...
    this.lastSig = 0; // last significant single ASCII char token, e.g. ')', ']', '}', '+', '-'
    this.lastTwo = 0; // (prev<<8)|curr for single-char tokens
  }
}
  ...
  tokenizeChunk(u8, st, emit){
    ...
    while (i < n){
      ...
      if (isIdentStart(c) || (c===0x5C && i+1<n && u8[i+1]===0x75)){
        ...
        const [kw, code] = isKWBytes(u8,s,i); push(kw?TT.Keyword:TT.Identifier, s, i, code); continue;
      }
      ...
      if (c===0x2F){ if (i+1<n){ const d=u8[i+1]; if (d===0x2F){ st.mode='lineCom'; i+=2; continue; } if (d===0x2A){ st.mode='blockCom'; i+=2; continue; } if (canStartRegex(st.prevType, st.prevKwCode, st.lastSig, st.lastTwo)){ st.mode='regex'; i+=1; st.rxInClass=false; continue; } }
        // operator '/' or '/='
        let s=i; i++; if (i<n && u8[i]===0x3D) i++; push(TT.Operator,s,i); continue; }
      ...
      { let s=i; const ch=c; i++; const n1=(i<n)?u8[i]:0, n2=(i+1<n)?u8[i+1]:0;
        ...
        if (end > s) {
          push(TT.Operator, s, end);
          if (end === s + 1) {
            st.lastTwo = ((st.lastTwo & 0xFF) << 8) | ch;
            st.lastSig = ch;
          }
          i = end;
          continue;
        }
        const punct=(ch===0x28||ch===0x29||ch===0x5B||ch===0x5D||ch===0x7B||ch===0x7D||ch===0x2C||ch===0x3B||ch===0x3A||ch===0x2E); push(punct?TT.Punct:TT.Operator,s,i); 
        if (i === s + 1) {
          st.lastTwo = ((st.lastTwo & 0xFF) << 8) | ch;
          st.lastSig = ch;
        }
...
export class JSLangSpec extends LanguageSpec {
  ...
  keywordLookup(u8,s,e){ return isKWBytes(u8,s,e); }
